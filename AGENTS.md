...
1. **Хазяин & Платформа (UI Layer — 8 твърди локализации чрез `next-intl`):**
   - 8-те основни туристически пазара: 🇬🇧 `en` (Default / Fallback), 🇧🇬 `bg`, 🇹🇷 `tr`, 🇪🇸 `es`, 🇫🇷 `fr`, 🇮🇹 `it`, 🇩🇪 `de`, 🇬🇷 `el`.
   - Селектор на езици в хедъра на началната страница и дашборда.
