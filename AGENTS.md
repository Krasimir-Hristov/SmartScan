...
- **Бизнес модел и Грануларно таксуване (Granular Per-Space Billing)**:
  - **Независим жизнен цикъл на обект**: Абонаментът се управлява на ниво конкретно пространство (`space_id`), а не глобално за целия профил. Хазяин/собственик с един профил може независимо да активира, паузира или прекратява абонамента за всеки отделен обект.
  - **Специфично ценообразуване по вертикали (Vertical-Specific Pricing)**:
    - 🏡 `SmartScan Stay` (Airbnb/вили): **€9 / месец на обект** (или €79 / година). Включва сезонен паузинг (безплатно запазване на данните извън сезона).
    - 🍽️ `SmartScan Menu` (Ресторанти/бистра): **€39 / месец на заведение**. Съобразено с високото ежедневно натоварване (десетки маси, стотици гости дневно, алергени, препоръки).
    - 🏢 `SmartScan RealEstate` (Имоти за продажба/наем): **€19 / месец на активен имот**. Таксува се само за периода на активната рекламна кампания до продажбата.
  - **Таблица `spaces` колони за Stripe**:
    - `stripe_subscription_id` (TEXT, уникален абонамент в Stripe за конкретното пространство).
    - `stripe_price_id` (TEXT, ценови идентификатор според `space_type`).
    - `subscription_status` (`trialing | active | past_due | paused | canceled`).
    - `trial_ends_at` (TIMESTAMPTZ, 14 дни безплатен пробен период без изискване на карта).
  - **Stripe Customer Portal**: Пълен самообслужващ се панел, позволяващ на хазяина да спира плащането за зимен/летен имот с 1 клик, без да засяга останалите си обекти.
...
## 4. Communication Modalities & Multilingual Standards (Strict Cost Control)

### A. Езикова архитектура (Dual-Layer Localization)
1. **Хазяин & Платформа (UI Layer — 10 твърди локализации чрез `next-intl`):**
   - 10-те основни туристически пазара: 🇬🇧 `en` (Default / Fallback), 🇧🇬 `bg`, 🇷🇴 `ro`, 🇬🇷 `el`, 🇷🇺 `ru`, 🇹🇷 `tr`, 🇩🇪 `de`, 🇪🇸 `es`, 🇮🇹 `it`, 🇫🇷 `fr`.
   - Селектор на езици в хедъра на началната страница, дашборда и PWA.
   - Автоматичен fallback към `en` при браузъри извън поддържания списък.

2. **Гост & AI Консиерж (Universal Polyglot Layer — 50+ езика):**
   - AI моделът (Gemini Flash) е универсален полиглот: автоматично засича езика на госта от съобщението или заглавията на устройството и отговаря на **същия език**.
   - Хазяинът въвежда наръчника само на своя роден език (напр. български), а консиержът го сервира свободно на немски, иврит, японски, френски и т.н.

### B. Модалности & Контрол на разходите
...
- **PDF Генерация**: `@react-pdf/renderer` за физически QR табелки с основен формат **A4** (опционално A5/A6), истински векторен QR код и векторни SVG знамена за 10-те езика, зареждан единствено на клиента (`ssr: false`).
