...
const CHECKOUT_OPTIONS = ['09:00', '10:00', '11:00', '12:00', '13:00', '14:00'];
...
export const SpaceEditor: React.FC<SpaceEditorProps> = ({
  ...
  const [nightSilenceStart, setNightSilenceStart] = useState(
    settings.nightSilenceStart || '23:00',
  );
  const [nightSilenceEnd, setNightSilenceEnd] = useState(
    settings.nightSilenceEnd || '08:00',
  );

  const [hasAfternoonRest, setHasAfternoonRest] = useState(
    Boolean(settings.afternoonRestStart && settings.afternoonRestEnd),
  );
  const [afternoonRestStart, setAfternoonRestStart] = useState(
    settings.afternoonRestStart || '14:30',
  );
  const [afternoonRestEnd, setAfternoonRestEnd] = useState(
    settings.afternoonRestEnd || '17:30',
  );

  // Check-in & Check-out Dropdowns
  const [checkInTime, setCheckInTime] = useState(
    settings.checkInTime || '15:00',
  );
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      const result = await updateSpaceAction({
        ...
        nightSilenceStart: hasNightSilence ? nightSilenceStart : '',
        nightSilenceEnd: hasNightSilence ? nightSilenceEnd : '',
        afternoonRestStart: hasAfternoonRest ? afternoonRestStart : '',
        afternoonRestEnd: hasAfternoonRest ? afternoonRestEnd : '',
