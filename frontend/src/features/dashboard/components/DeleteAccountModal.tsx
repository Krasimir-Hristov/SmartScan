...
export const DeleteAccountModal: React.FC<DeleteAccountModalProps> = ({
  ...
  const handleDelete = async () => {
    ...
    try {
      const result = await deleteAccountAction(typedEmail);
      if (!result.success) {
        setErrorMessage(result.error || t('deleteModalErrorServer'));
        setIsDeleting(false);
        return;
      }
      ...
      setErrorMessage(t('deleteModalErrorServer'));
      setIsDeleting(false);
    }
  };
  ...
  const modalContent = (
    ...
    >
      ...
      >
        ...
        {/* Instructions & Email Input (GitHub-Style) */}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-2">
          ...
          <button
            ...
            aria-label={t('deleteModalConfirmButton')}
          ...
          >
            {isDeleting ? (
              <>
                ...
                <span>{t('deleteModalDeleting')}</span>
              </>
            ) : (
              <>
                <Trash2 className="w-4 h-4" />
                <span>{t('deleteModalConfirmButton')}</span>
              </>
            )}
          </button>
        </div>
