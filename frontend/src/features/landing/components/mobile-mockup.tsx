...
interface ChatMessage {
  id: string;
  sender: 'guest' | 'ai';
  textKey?: 'chatGuestQuestion' | 'chatAiResponse';
  customText?: string;
  time: string;
}
...
export const MobileMockup: React.FC<MobileMockupProps> = ({ onOpenDemo }) => {
  ...
  const [chatHistory, setChatHistory] = useState<ChatMessage[]>([
    {
      id: 'msg-1',
      sender: 'guest',
      textKey: 'chatGuestQuestion',
      time: '19:42',
    },
    {
      id: 'msg-2',
      sender: 'ai',
      textKey: 'chatAiResponse',
      time: '19:42',
    },
  ]);
  ...
  const handleCopyWifi = async () => {
    triggerHaptic(60);
    try {
      await navigator.clipboard.writeText('Sanctuary_5G_Guest_2026');
      setCopiedWifi(true);
      setTimeout(() => setCopiedWifi(false), 2200);
    } catch {
      // If clipboard write is rejected, do not toggle copied status
    }
  };
    ...
    <div className="flex flex-col justify-between rounded-3xl bg-zinc-950/80 border border-emerald-500/20 p-6 sm:p-8 shadow-2xl shadow-emerald-950/20 hover:border-emerald-500/40 transition-all">
        ...
        <div className="rounded-2xl bg-zinc-900 border border-zinc-800 p-4 sm:p-5 shadow-inner">
          ...
          <div className="py-3 flex flex-wrap gap-2">
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium bg-emerald-500/10 hover:bg-emerald-500/20 text-emerald-300 border border-emerald-500/30 transition-all cursor-pointer disabled:cursor-not-allowed active:scale-95"
            >
          ...
          <div className="flex flex-col gap-3 py-2 max-h-52 overflow-y-auto pr-1">
            {chatHistory.map((msg) => {
              const text = msg.textKey ? t(msg.textKey) : msg.customText;
              return (
                <div
                  key={msg.id}
          ...
          <form onSubmit={handleSendSample} className="mt-3 relative flex items-center">
            ...
            <button
              ...
              className="absolute right-1.5 p-1.5 rounded-lg bg-emerald-500 text-zinc-950 hover:bg-emerald-400 transition-colors cursor-pointer disabled:cursor-not-allowed"
            >
      ...
      <div className="mt-6 pt-6 border-t border-zinc-900 flex items-center justify-between">
        ...
        <button
          ...
          className="text-xs font-semibold text-emerald-400 hover:text-emerald-300 transition-colors cursor-pointer disabled:cursor-not-allowed"
        >
          {t('mobileAction')}
        </button>
      </div>
