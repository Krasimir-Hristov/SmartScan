...
### [x] Стъпка 8: Физически QR Табелки за печат (`design/02`)

- [x] **Стъпка 8А**: Интерактивен двуколонен модал в Дашборда (`QrPrintModal`) с жив WYSIWYG преглед на табелката.
- [x] **Стъпка 8А**: Основен европейски формат по подразбиране **A4** (210×297mm) с бързи опции за A5 (148×210mm) и A6 (105×148mm).
- [x] **Стъпка 8А**: Пълна забрана на емоджита — използване само на официални векторни SVG знамена от `flag-icons` и векторни икони от `lucide-react`.
- [x] **Стъпка 8А**: Минималистичен дизайн без претоварване: премахнати са Wi-Fi пароли, телефони, адреси и технически надписи ("Print spec 300 DPI") от лицето на табелката.
- [x] **Стъпка 8А**: 10 туристически езика (EN, BG, DE, RO, EL, RU, TR, ES, IT, FR) със съответните знамена и кратки призиви.
- [x] **Стъпка 8А**: Две теми: **Paper-Friendly Light** (бяла хартия/пестене на мастило) и **Obsidian Dark Luxury** (тъмен акрил).
- [x] **Стъпка 8А**: Двуколонен модален модал (`PlaqueModalShell`, portal + focus trap + ESC) — split view: контроли вляво, жив WYSIWYG преглед вдясно.
- [x] **Стъпка 8А**: Директен печат (`window.print()`) с истински печатни стилове: динамичен `@page { size: <формат>; margin: 0 }`, `print-color-adjust: exact` (тъмният фон и SVG знамената се печатат), mm-точно позициониране на табелката и `transform: scale()` за 1:1 WYSIWYG без втора празна страница.
- [x] **Стъпка 8А**: Сваляне на самостоятелен PNG QR код (1024×1024) с quiet zone от 4 модула и име на файла, базирано на slug.
- [x] **Стъпка 8А**: Единен източник на истина за табелката (`lib/plaqueConfig.ts`): 10-те езика, знамената, форматите, размерите и чистите helper-и (без дублиране между preview и PDF).
- [x] **Стъпка 8А**: Печатните размери и формати са строго типизирани (`PlaquePrintSpec`) — A4 по подразбиране, A5/A6 с точно съотношение на страницата.
- [x] **Стъпка 8Б**: Векторен PDF генератор с `@react-pdf/renderer`, зареден динамично на клиента (`ssr: false`).
- [x] **Стъпка 8Б**: **Истински векторен QR код** в PDF-а — модулната геометрия се извлича от рендирания QR (`<Svg><Path/></Svg>`), с raster fallback (1200px PNG) при невъзможност.
- [x] **Стъпка 8Б**: **Векторни знамена в PDF-а** (`PlaquePdfFlag`) вместо текстови хапчета — GB, BG, DE, RO, GR, RU, TR, ES, IT, FR, рисувани с react-pdf `<Svg>`/`<Rect>`/`<Line>`/`<Polygon>` примитиви (без емоджита и без външни assets).
- [x] **Стъпка 8Б**: Стиловете на PDF-а са изнесени в `lib/plaquePdfStyles.ts` (палитра, метрики по формат, memoized `StyleSheet`), файловете са под 300 реда.
- [x] **Стъпка 8Б**: Локално интегрирани Unicode TTF шрифтове (Roboto) за поддръжка на кирилица (Български, Русский), гръцки (Ελληνικά) и латиница — само локален `public/fonts/` път, без CDN зависимост.
- [x] **Стъпка 8Б**: Сваляне на готови производствени PDF файлове: `SmartScan_[Slug]_[Format]_[Theme].pdf`.
- [x] **Стъпка 8А/8Б**: Каноничен публичен домейн за QR кода (`NEXT_PUBLIC_SITE_URL` → валидиран request host), подаван от server component; предупреждение в UI при локален/preview адрес, за да не се печата неизползваема табелка.
- [x] **Стъпка 8А/8Б**: Ясна обратна връзка при грешка (`qrError`, `pdfError`) и локализирани loading/skeleton състояния в 10-те езика.
- [x] **Стъпка 8А**: Премахнат е неизползваният emoji `flag` ключ от `lib/i18n/config.ts` (0 емоджита в кода и текстовете на табелката).
- [x] **Отклонение**: Планът предвиждаше бутон и в `PropertyCard.tsx`, който е неизползван mock компонент без реални `Space` данни — бутонът остава само в `SpaceEditor.tsx` и `PropertyCard` е кандидат за премахване в Стъпка 10.
- [x] **Верификация**: `npx tsc --noEmit` = 0 грешки, `npm run lint` = 0 грешки, `npm run build` = 0 грешки, паритет на 22+3 i18n ключа в 10-те локала.
