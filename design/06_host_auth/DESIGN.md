...
- **Primary Method**: 1-Click Google OAuth (Supabase Auth)
...
| `accent-emerald` | `#10B981` | Cyber Emerald (Active tab dot, Trust badges, Glow effects) |
...
3. **Primary Action (1-Click Google OAuth)**:
   - Full-width dark frosted button with Cyber Emerald border on hover: **`Continue with Google`** (official colored Google 'G' icon).
   - Instant authentication without passwords via Supabase OAuth.
4. **Security & Value Props**:
   - `⚡ Zero passwords to remember · 1-Click instant sign-in with Google`.
