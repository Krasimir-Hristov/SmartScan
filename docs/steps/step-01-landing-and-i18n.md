...
2. **Модулни Feature-based компоненти**:
   - `LandingHero.tsx`: Силно първо послание с двойно CTA (`Start Free with Google` и демо за гости).
   - `HeroShowcase.tsx`, `PlaqueMockup.tsx`, `MobileMockup.tsx`: Интерактивни визуализации на физическата акрилна табелка и мобилното PWA за гости.
   - `HowItWorks.tsx`: Информационна решетка от 3 стъпки (Гласово въвеждане ➔ Печат на табелка ➔ Гост сканира).
   - `FeaturesGrid.tsx`: Карти за нулева халюцинация (100% верифицирани данни), свръхбърз SSE стрийминг и спешна ескалация към хазяина по WhatsApp/телефон.
   - `PricingSection.tsx`: Прозрачен ценови модел (€9/месец на обект или €79/година с безплатно сезонно паузиране).
   - `AuthModal.tsx`: Модал за бърз вход с Google OAuth или директно тестване на демото.
